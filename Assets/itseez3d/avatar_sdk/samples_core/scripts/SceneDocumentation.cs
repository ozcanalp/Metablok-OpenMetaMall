	...
	public class SceneDocumentation : MonoBehaviour
	{
		...
		public void OpenDocumentationForScene()
		{
			if (Application.isEditor) {
				...
				if (sceneName.EndsWith("_local_compute"))
				{
					flavour = Flavour.LOCAL_COMPUTE;
				}
				if (sceneName.EndsWith("_cloud") || sceneName.EndsWith("_local_compute")) {
					sceneName = sceneName.Replace("_cloud", "");
					sceneName = sceneName.Replace("_local_compute", "");
				}

				DocumentationHelper.OpenDocumentationInBrowser(string.Format("scene_{0}.html", sceneName), flavour);
			}
		}
	}
