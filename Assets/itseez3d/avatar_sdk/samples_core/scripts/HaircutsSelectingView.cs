...
namespace ItSeez3D.AvatarSdkSamples.Core
{
	...
	public class HaircutsSelectingView : ItemsSelectingView
	{
		...
		public void InitItems(string avatarCode, List<string> items, IAvatarProvider avatarProvider)
		{
			...
			{
				Text statusText = UnityUtils.FindSubobjectByName(t.gameObject, "StatusText").GetComponentInChildren<Text>();

				string haircutId = t.GetComponentInChildren<ToggleId>().Text;
				var traits = PipelineSampleTraitsFactory.Instance.GetTraitsFromAvatarCode(avatarCode);

				if (!traits.HaircutNeedsPreview(haircutId))
				{
			...
			foreach (Toggle t in previewToggles)
			{
				if (!t.IsDestroyed())
				{
					...
					else
					{
						...
						if (!t.IsDestroyed())
						{
							GameObject backgroundObject = UnityUtils.FindSubobjectByName(t.gameObject, "Background");
