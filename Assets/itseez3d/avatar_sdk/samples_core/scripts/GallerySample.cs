...
namespace ItSeez3D.AvatarSdkSamples.Core
{
	...
	public abstract class GallerySample : MonoBehaviour, IAvatarClickHandler
	{
		...
		// Drop down to select pipeline type
		...
		void Start()
		{
			...
			pipelineType = selector.PipelineType;
			...
			if (webCameraCapturer != null)
				webCameraCapturer.OnPhotoMade += bytes => StartCoroutine(CreateNewAvatar(bytes));
		}
		...
		private IEnumerator Initialize()
		{
			...
			{
				AvatarSdkMgr.Init(stringMgr: stringManager, storage: persistentStorage, sdkType: sdkType);
			}
		...
		protected virtual void InitAvatarPreview(AvatarPreview preview, string avatarCode, GalleryAvatarState avatarState)
		{
			...
			var pipelineTraits =  PipelineTraitsFactory.Instance.GetTraitsFromAvatarCode(avatarCode);
			if (pipelineTraits != null)
				preview.UpdatePipelineText(pipelineTraits.DisplayName);
		}
		...
		public void OnPipelineTypeToggleChanged(PipelineType pipeline)
		{
