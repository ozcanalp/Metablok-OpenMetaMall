...
namespace GLTF
{
	public static class GLTFHelpers
	{
		...
		public static void BuildMeshAttributes(ref Dictionary<string, AttributeAccessor> attributes, AssetCache assetCache = null)
		{
			...
			foreach (var kvp in attributes)
			{
				...
				BufferViewCacheData bufferViewCache = LoadBufferView(attributeAccessor, assetCache);
				switch (kvp.Key)
				{
					case SemanticProperties.POSITION:
						attributeAccessor.AccessorId.Value.AsVertexArray(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
						break;
					case SemanticProperties.NORMAL:
						attributeAccessor.AccessorId.Value.AsNormalArray(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
						break;
					case SemanticProperties.TANGENT:
						attributeAccessor.AccessorId.Value.AsTangentArray(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
						break;
					...
					case SemanticProperties.TEXCOORD_3:
						attributeAccessor.AccessorId.Value.AsTexcoordArray(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
						break;
					case SemanticProperties.COLOR_0:
						attributeAccessor.AccessorId.Value.AsColorArray(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
						break;
					case SemanticProperties.WEIGHTS_0:
					case SemanticProperties.JOINTS_0:
						attributeAccessor.AccessorId.Value.AsVector4Array(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
						break;
					case SemanticProperties.INDICES:
						attributeAccessor.AccessorId.Value.AsUIntArray(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
						break;
					default:
		...
		public static void BuildTargetAttributes(ref Dictionary<string, AttributeAccessor> attributes, AssetCache assetCache = null)
		{
			foreach (var kvp in attributes)
			{
				var attributeAccessor = kvp.Value;
				...
				switch (kvp.Key)
				{
					...
					case SemanticProperties.NORMAL:
					case SemanticProperties.TANGENT:
						attributeAccessor.AccessorId.Value.AsVector3Array(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
						break;
					default:
		...
		public static void BuildBindPoseSamplers(ref AttributeAccessor attributeAccessor, AssetCache assetCache = null)
		{
			NumericArray resultArray = attributeAccessor.AccessorContent;
			BufferViewCacheData bufferViewCache = LoadBufferView(attributeAccessor, assetCache);
			attributeAccessor.AccessorId.Value.AsMatrix4x4Array(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
			attributeAccessor.AccessorContent = resultArray;
		}
		...
		public static void BuildAnimationSamplers(ref Dictionary<string, List<AttributeAccessor>> samplers, AssetCache assetCache = null)
		{
			foreach (var samplerSet in samplers)
			{
				foreach (var attributeAccessor in samplerSet.Value)
				{
					...
					switch (samplerSet.Key)
					{
						case "time":
						case "weights":
							attributeAccessor.AccessorId.Value.AsFloatArray(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
							break;
						case "translation":
						case "scale":
							attributeAccessor.AccessorId.Value.AsVector3Array(ref resultArray, bufferViewCache.data, bufferViewCache.Offset);
		...
		private static BufferViewCacheData LoadBufferView(AttributeAccessor attributeAccessor, AssetCache assetCache = null)
		{
			BufferView bufferView = attributeAccessor.AccessorId.Value.BufferView.Value;
			if (assetCache != null && assetCache.BufferViewCache[attributeAccessor.AccessorId.Value.BufferView.Id] != null)
				return assetCache.BufferViewCache[attributeAccessor.AccessorId.Value.BufferView.Id];

			BufferViewCacheData bufferViewCache = new BufferViewCacheData();
			...
			if (attributeAccessor.Stream is System.IO.MemoryStream)
			{
				...
				bufferViewCache.data = memoryStream.GetBuffer();
				bufferViewCache.Offset = totalOffset;
			}
			else
			{
				attributeAccessor.Stream.Position = totalOffset;
				bufferViewCache.data = new byte[bufferView.ByteLength];
				...
				while (remainingSize != 0)
				{
					...
					attributeAccessor.Stream.Read(bufferViewCache.data, (int)(bufferView.ByteLength - remainingSize), sizeToLoad);
					remainingSize -= (uint)sizeToLoad;
				}
			}

			if (assetCache != null)
				assetCache.BufferViewCache[attributeAccessor.AccessorId.Value.BufferView.Id] = bufferViewCache;
		...
		private static uint LoadBufferView(AttributeAccessor attributeAccessor, out byte[] bufferViewCache)
		{
			...
			if (bufferView.ByteLength == 42637728)
				Debug.LogWarningFormat("[DBG] Allocated bytes: {0}", bufferView.ByteLength);
