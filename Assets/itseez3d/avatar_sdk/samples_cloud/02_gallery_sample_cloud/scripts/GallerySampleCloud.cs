...
namespace ItSeez3D.AvatarSdkSamples.Cloud
{
	public class GallerySampleCloud : GallerySample
	{
		...
		private IEnumerator GetAllAvatarsFunc(int maxItems, AsyncRequest<GalleryAvatar[]> request)
		{
			...
			//Don't show fullbody avatars in the Gallery sample
			List<GalleryAvatar> avatars = new List<GalleryAvatar>();
			for (int i=0; i< avatarsRequest.Result.Length; i++)
			{
				AvatarData avatarData = avatarsRequest.Result[i];
				if (!avatarData.pipeline.Contains("body"))
					avatars.Add(new GalleryAvatarCloud() { code = avatarData.code, state = GetAvatarState(avatarData), avatarData = avatarData });
			}

			request.Result = avatars.ToArray() ;
			request.IsDone = true;
		}
